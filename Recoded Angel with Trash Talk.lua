...
local menu = {
    ...
    custom = {
        header = ui.new_label("Rage", "Other", "Paramedic Resolver"),
...
local function on_paint()
    ...
    if show_statistics then
        ...
        renderer.text(x_offset, y_offset, ui_colors.accent.r, ui_colors.accent.g, ui_colors.accent.b, ui_colors.accent.a, "", 0, "Paramedic Statistics")
...
-- This lua is created my stack https://discord.gg/QPSHQNTFER and paramedic
...
client.set_event_callback('aim_hit', function(e)
    ...
    if debug_logs then
        client.color_log(ui_colors.success.r, ui_colors.success.g, ui_colors.success.b,
            string.format('[Paramedic] Hit %s in the %s for %d damage (%d health remaining) (Side: %s | Desync: %.1f° | Method: %s)',
...
client.set_event_callback('aim_miss', function(e)
    ...
    if debug_logs then
        client.color_log(ui_colors.warning.r, ui_colors.warning.g, ui_colors.warning.b,
            string.format('[Paramedic] Missed %s (%s) due to %s | Detailed: %s (Side: %s | Desync: %.1f° | Method: %s)',
            entity.get_player_name(player),
            group,
            e.reason,
            detailed_reason,
...
client.color_log(ui_colors.highlight.r, ui_colors.highlight.g, ui_colors.highlight.b, "[Paramedic] Configuration available in RAGE > Other tab")
client.color_log(ui_colors.highlight.r, ui_colors.highlight.g, ui_colors.highlight.b, "[Paramedic] Recoded by Paramedic - The most resolver available")

local function report_statistics()
    if ui.get(menu.custom.enable) then
        local hit_rate = resolver_stats.total_hits / math.max(resolver_stats.total_hits + resolver_stats.total_misses, 1)
        
        client.color_log(ui_colors.accent.r, ui_colors.accent.g, ui_colors.accent.b, string.format("[Paramedic] Performance Report - Accuracy: %.1f%% (%d hits, %d misses)", hit_rate * 100, resolver_stats.total_hits, resolver_stats.total_misses))
        ...
        if best_method ~= "none" then
            client.color_log(ui_colors.success.r, ui_colors.success.g, ui_colors.success.b, string.format("[Paramedic] Most effective method: %s (%.1f%% accuracy)", best_method, best_accuracy * 100))
        end
    end
...
client.set_event_callback("paint_ui", function()
    ...
    local fade_text = fade_label(time, "Paramedic Resolver", 159, 202, 11, 255)
...
-- Trash Talk
local trash_talk_enabled = ui.new_checkbox("Rage", "Other", "Trash Talk on kill")
local death_say_enabled = ui.new_checkbox("Rage", "Other", "Trash Talk on death")

local kill_phrases = {
    "тебя убил paramedic resolver .gg/QPSHQNTFER", "get good get paramedic resolver (.gg/QPSHQNTFER)", "owned by paramedic resol", "говно хватит умирать лучше купи норм резик(.gg/QPSHQNTFER)", "go uninstall you poor resol and go download best resol", "ur mad?", "че запредиктили тебя лошпед?", "плачь больше", "как же легко", "go uninstall",
    "nt bro", "get clapped", "sit down", "nice try, bot", "just better", "gg ez"
}

local death_phrases = {
    "тебе повезло нищита", "как же тебе везет", "мой ресольвер дал тебе шанс используй его", "не этот сочник пикнет", "что ты сделал идиот", "найс идиот что ты сделал", "что за луа на аашки используешь?"
}
...
client.set_event_callback("player_death", function(e)
    ...
    -- Trash talk on kill
    if ui.get(trash_talk_enabled) and attacker == local_player and victim ~= local_player then
        local phrase = kill_phrases[client.random_int(1, #kill_phrases)]
        client.exec("say " .. phrase)
    end

    -- Trash talk on death
    if ui.get(death_say_enabled) and victim == local_player and attacker ~= local_player then
        local phrase = death_phrases[client.random_int(1, #death_phrases)]
        client.exec("say " .. phrase)
    end
end)